...
async def get_ai_providers():
            ...
            if mongo_providers:
                for provider in mongo_providers:
                    providers_data.append({
                        "id": provider.get("provider_id", str(provider.get("_id"))),
    ...
    # Fallback to static provider list
    if not providers_data:
        providers_data = [
            {"id": "openai", "name": "OpenAI", "type": "text", "status": "active", "description": "GPT models for text generation"},
            {"id": "stability-ai", "name": "Stability AI", "type": "image", "status": "active", "description": "Stable Diffusion for image generation"},
            {"id": "anthropic", "name": "Anthropic", "type": "text", "status": "active", "description": "Claude models for text generation"},
            {"id": "midjourney", "name": "Midjourney", "type": "image", "status": "active", "description": "AI image generation platform"},
            {"id": "dalle", "name": "DALL-E", "type": "image", "status": "active", "description": "OpenAI's image generation model"},
            {"id": "google-palm", "name": "Google PaLM", "type": "text", "status": "active", "description": "Google's language model"},
            {"id": "cohere", "name": "Cohere", "type": "text", "status": "active", "description": "Natural language processing platform"},
            {"id": "huggingface", "name": "Hugging Face", "type": "both", "status": "active", "description": "Open source AI models"},
            {"id": "replicate", "name": "Replicate", "type": "both", "status": "active", "description": "Machine learning model hosting"}
        ]
...
async def validate_provider_api_key(request: dict):
    """Validate AI provider API key (alternative endpoint)"""
...
async def get_recommended_providers():
    ...
    return {
        "recommended": [
            {"id": "openai", "name": "OpenAI", "reason": "Best for text generation and DALL-E images", "priority": 1},
            {"id": "stability-ai", "name": "Stability AI", "reason": "Excellent for high-quality image generation", "priority": 2}
        ]
    }
...
async def update_provider_configuration(provider_id: str, config: dict):
    ...
    return {"success": True, "message": f"Configuration updated successfully for {provider_id}"}
...
async def login(credentials: dict = None):
    ...
    return {
        ...
        "token_type": "bearer",
        "user": {"id": "1", "email": "user@company.com", "name": "Test User"}
    }

@app.get("/api/v1/auth/me")
async def get_current_user():
    """Mock current user endpoint"""
    return {"id": "1", "email": "user@company.com", "name": "Test User", "company": "Test Company"}

if __name__ == "__main__":
    print("🚀 Starting AI Marketing Agent API")
    print(f"📊 Database: MongoDB ({'Connected' if mongodb_connected else 'Disconnected'})")
